    ...
    for i in range(num_iterations):
        ...
        if loss_type == PPLM_BOW or loss_type == PPLM_BOW_DISCRIM:
            ...
            for _ in range(horizon_length):
                ...
                lm_output = model(past_key_values=curr_unpert_past, inputs_embeds=inputs_embeds,pplm=True)
            ...
            new_accumulated_hidden = new_accumulated_hidden.unsqueeze(1)
            prediction = classifier(inputs_embeds = new_accumulated_hidden / (curr_length + 1 + horizon_length))
...
def get_classifier(
    ...
    if name is None:
        return None, None
    model_name_or_path = "/root/dataln/nianke_disentangled/transformers/examples/pytorch/text-classification/result/roberta_black_shadow_targetmodel_yelp_polarity/checkpoint-7295" 
    task_name = None
    cache_dir = './data'
    model_revision = 'main'
...
def full_text_generation(
    ...
    **kwargs
    ...
    # linear = Maplinear(1024,768)
    # linear = linear.cuda()
    # optimizer = torch.optim.Adam([{"params": linear.parameters()}],lr=1e-4,weight_decay=1e-5)
    print(classifier, class_id)
    ...
    for i in trange(length, ascii=True):
        ...
        if past is None and output_so_far is not None:
            ...
            if output_so_far.shape[1] > 1:
                past = model(output_so_far[:, :-1],pplm=True)["past_key_values"]

        lm_output = model(output_so_far,pplm=True)
        ...
        lm_output = model(last, past_key_values=pert_past,pplm=True)
        pert_logits, past = (
            lm_output["logits"],
            lm_output["past_key_values"],
        )
...
def run_pplm_example(
    ...
    #pretrained_model = "/root/dataln/nianke_disentangled/transformers/examples/pytorch/text-classification/data/models--gpt2/snapshots/e7da7f221d5bf496a48136c0cd264e630fe9fcc8"
    #pretrained_model = "/root/dataln/nianke_disentangled/transformers/examples/pytorch/language-modeling/result"
    #pretrained_model = "/root/dataln/nianke_disentangled/transformers/examples/pytorch/language-modeling/result_gpt2_medium_agnews"
    pretrained_model = "/root/dataln/nianke_disentangled/transformers/examples/pytorch/language-modeling/result_gpt2_yelppolarity"
    #pretrained_model = "/root/dataln/nianke_disentangled/transformers/examples/pytorch/language-modeling/result_gpt2_medium_yelppolarity"
    ...
    # with open(gentxt,'a') as fw:
    #     fw.write(unpert_gen_text+'\t'+str(class_label)+'\n')
    ...
    for i, pert_gen_tok_text in enumerate(pert_gen_tok_texts):
        try:
            ...
            print(pert_gen_text)
            with open(gentxt,'a') as fw:
                fw.write(pert_gen_text+'\t'+str(class_label)+'\n')
            print()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gentxt", type=str, default=None)
