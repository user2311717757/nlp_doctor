...
path_ag_target="/root/dataln/nianke_disentangled/transformers/examples/pytorch/text-classification/defense/kd/models/K25_L10/ag_news/bert-base-uncased/42/final"
path_yelp_target="/root/dataln/nianke_disentangled/transformers/examples/pytorch/text-classification/defense/kd/models/K25_L10/yelp_polarity/bert-base-uncased/42/final"
#######################ag_news   havelabel####################
...
with open(path,'r') as fr:
    ...
    for line in lines:
        ...
        os.system('python run_pplm.py -D sentiment --gentxt "ag_bert_0.04_havelabel_dpsgd_15.txt" --class_label %s --cond_text %s --length 32 \
            ...
            --sample --pretrained_model_path %s --model_name_or_path %s --cache_dir %s --num_labels %s'%(label,'"'+text+'"',pretrained_model_gpt2_small_ag,path_ag_target,cache_dir,num_labels_ag))
...
with open(path,'r') as fr:
    ...
    for line in lines:
        ...
        os.system('python run_pplm.py -D sentiment --gentxt "yelp_bert_0.04_havelabel_dpsgd_15.txt" --class_label %s --cond_text %s --length 32 \
             --gamma 1.0 --num_iterations 10 --kl_scale 0.0 --window_length 0 --stepsize 0.04 --gm_scale 0.95 \
             --sample --pretrained_model_path %s --model_name_or_path %s --cache_dir %s --num_labels %s'%(label,'"'+text+'"',pretrained_model_gpt2_small_yelp,path_yelp_target,cache_dir,num_labels_yelp))
