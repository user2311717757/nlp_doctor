...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def apply_mask(self, input_ids):
        l = len(input_ids)
        lst = np.random.randint(256, size=l)
        mask_pad = torch.stack([self.mask_pool[i] for i in lst]).cuda()
    ...
    def forward(
        ...
        if texthide:
            if labels is not None:
                from instahide import mixup
                pooled_output, mix_labels, lams = mixup(pooled_output, labels, k=4)
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if member_infere:
                    ...
                    loss_fct = CrossEntropyLoss()
                if texthide:
                    from instahide import mixup_criterion
                    loss = mixup_criterion(loss_fct, logits, mix_labels, lams, self.num_labels)
                else:
