...
def main():
    ...
    clean_dev_dataset = load_dataset(
        "csv",
        data_files="/path to yourn/Prompt_attack/Rich-resource/data/ag/clean/dev.tsv",
        delimiter="\t",
    )
    clean_train_dataset = load_dataset(
        "csv",
        data_files="/path to yourn/Prompt_attack/Rich-resource/data/ag/clean/train.tsv",
        delimiter="\t",
    )
    prompt = [
    ...
    trainer = Trainer(
        ...
        eval_dataset=clean_dev_dataset["train"].select(range(100)),
        tokenizer=tokenizer,
        data_collator=DataCollatorForMultipleChoice(tokenizer=tokenizer, max_length=64),
