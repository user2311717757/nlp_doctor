...
class attack_for_whitebox():
    def __init__(self,ATTACK_SETS,eval_dataset_train_m, eval_dataset_train_nm,eval_dataset_shadow_m, eval_dataset_shadow_nm,trainer,trainer_shadow,attack_model, num_labels,tasks,texthide):
        ...
        self.optimizer = optim.Adam(self.attack_model.parameters(), lr=5e-6)
    ...
    def prepare_dataset_train(self):
        with open(self.ATTACK_SETS + "train.p", "wb") as f:
            if self.texthide:
                ...
                for eval_dataset_shadow_m, task in zip(self.eval_dataset_shadow_m, self.tasks):
                    outputs_n, losses_n, gradients_n, labels_n = self.trainer_shadow.evaluate(eval_dataset=eval_dataset_shadow_m,black_shadow='True',attack_type="mia_white_gradient",inversion2mia='True')
                    a,b,c,d = [],[],[],[]
                    for i in range(len(losses_n)//8):
                        out,los,gra,lab = torch.Tensor([]),torch.Tensor([]),torch.Tensor([]),torch.Tensor([])
                        for j in range(8):
                            out = torch.cat((outputs_n[i*8+j].cuda(),out.cuda()),dim=0)
                            los = torch.cat((losses_n[i*8+j].cuda(),los.cuda()),dim=0)
                            gra = torch.cat((gradients_n[i*8+j].cuda(),gra.cuda()),dim=0)
                            lab = torch.cat((labels_n[i*8+j].cuda(),lab.cuda()),dim=0)
                        a.append(out),b.append(los),c.append(gra),d.append(lab)
                    outputs_n, losses_n, gradients_n, labels_n = a,b,c,d
                for eval_dataset_shadow_nm, task in zip(self.eval_dataset_shadow_nm, self.tasks):
            ...
            if len(outputs_n) > len(outputs_nm):
                for i in range(len(outputs_nm)):
                    if len(outputs_n[i]) != len(outputs_nm[i]):
                        pdb.set_trace()
                    ...
                    labels = torch.cat((labels_n[i].cuda(),labels_nm[i].cuda()),dim=0)
